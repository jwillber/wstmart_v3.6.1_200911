...
class Orders extends Base{
	...
	public function shopOrdersByPage($orderStatus, $sId=0){
		...
		$page = $this->alias('o')->where($where)->where($condition)
		      ...
		      ->field('o.orderRemarks,o.noticeDeliver,o.orderId,orderNo,goodsMoney,totalMoney,realTotalMoney,orderStatus,deliverType,deliverMoney,isAppraise,isRefund,o.deliverType deliverTypes
		              ,payType,payFrom,userAddress,orderStatus,isPay,isAppraise,userName,userPhone,orderSrc,o.createTime,orf.id refundId,o.orderCode')
			  ->order('o.createTime', 'desc')
			  ->paginate()->toArray();
	    if(count($page['data'])>0){
	    	 $orderIds = [];
	...
	public function getByView($orderId, $uId=0){
		...
		$orders = Db::name('orders')->alias('o')
		               ...
		               ->field('o.*,s.areaId shopAreaId,s.shopAddress,s.shopTel,s.shopName,s.shopQQ,s.shopWangWang,orf.id refundId,orf.refundRemark,orf.refundStatus,orf.refundTime,orf.backMoney,orf.backMoney,oc.complainId,o.orderCode')->find();
	...
	*/
	...
	public function getVerificatOrder($shopId,$storeId=0){
		...
		$order = Db::name('orders')
		               ->where($where)
		               ->field('orderId,orderNo,userPhone,userName,isPay,goodsMoney,totalMoney,realTotalMoney,isInvoice,invoiceClient,orderRemarks,scoreMoney,useScore,orderStatus,createTime,verificationTime,isMakeInvoice,userCouponId,userCouponJson')
		               ->find();
		if(empty($order)){
			return $order ;
		}
