...
function initGrid(p){
    ...
    var cols = [
            ...
            {title:'提现类型', name:'accType' ,width:40,renderer:function(val,item,rowIndex){
                if(item['accType']==1)return '微信';
                if(item['accType']==2)return '支付宝';
                if(item['accType']==3)return '银行卡';
            }},
            {title:'会员类型', name:'targetTypeName' ,width:50,sortable: true},
            {title:'会员名称', name:'loginName' ,width:50, renderer:function(val,item,rowIndex){
                return WST.blank(item['userName'])+"("+item['loginName']+")";
            }},
            {title:'提现账号信息', name:'accTargetName' ,width:280, renderer:function(val,item,rowIndex){
                if(item['accType']==1){
                    return item['accNo'];
                }else{
            ...
            {title:'持卡人/姓名', name:'accUser' ,width:40,sortable: true},
            {title:'提现金额', name:'money' ,width:40,sortable: true, renderer:function(val,item,rowIndex){
                return '￥'+val;
            }},
            {title:'申请时间', name:'createTime',sortable: true ,width:60},
